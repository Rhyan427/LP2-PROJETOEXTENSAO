...
public class InscricaoController {
    ...
    @ResponseStatus(HttpStatus.OK)
    public List<Inscricao> verInscricoesPorDiscente(@PathVariable Integer idDiscente) {
        ...
        return inscricaoService.verInscricoesPorDiscente(discente);
    }
    ...
    @PostMapping("/{idDiscente}/inscrever/{idOportunidade}")
    @ResponseStatus(HttpStatus.CREATED)
    public Inscricao fazerInscricao(@PathVariable Integer idDiscente, @PathVariable Integer idOportunidade) {
        Discente discente = discenteService.buscarPorId(idDiscente)
                .orElseThrow(()-> new ResponseStatusException(HttpStatus.NOT_FOUND, "Discente nao encontrado."));
        Oportunidade oportunidade = oportunidadeService.buscarPorId(idOportunidade)
                .orElseThrow(()-> new ResponseStatusException(HttpStatus.NOT_FOUND, "Oportunidade nao encontrada."));
        return inscricaoService.fazerInscricao(discente, oportunidade);
    }

    @PatchMapping("/{idInscricao}/analisar_inscricao")
    @ResponseStatus(HttpStatus.ACCEPTED)
    public boolean analisarInscricao(@PathVariable Integer idInscricao, @RequestBody StatusInscricao status) {
